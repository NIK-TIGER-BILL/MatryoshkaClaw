---
summary: "Run MatryoshkaClaw in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want MatryoshkaClaw isolated from your main macOS environment
...
# MatryoshkaClaw on macOS VMs (Sandboxing)
...
Run MatryoshkaClaw in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
5. SSH in, install MatryoshkaClaw, configure channels
6. Done
...
The VM runs in the background. MatryoshkaClaw's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "matryoshka status"
```
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to MatryoshkaClaw.
...
Add to your MatryoshkaClaw config:
