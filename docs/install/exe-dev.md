---
summary: "Run MatryoshkaClaw Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: MatryoshkaClaw Gateway running on an exe.dev VM, reachable from your laptop via: `https://<vm-name>.exe.xyz`
...
Shelley, [exe.dev](https://exe.dev)'s agent, can install MatryoshkaClaw instantly with our
...
Tip: keep this VM **stateful**. MatryoshkaClaw stores state under `~/.openclaw/` and `~/.openclaw/workspace/`.
...
Run the MatryoshkaClaw install script:

```bash
curl -fsSL https://github.com/NIK-TIGER-BILL/MatryoshkaClaw/install.sh | bash
```

## 4) Setup nginx to proxy MatryoshkaClaw to port 8000
