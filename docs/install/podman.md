---
summary: "Run MatryoshkaClaw in a rootless Podman container"
read_when:
...
Run the MatryoshkaClaw gateway in a **rootless** Podman container. Uses the same image as Docker (build from the repo [Dockerfile](https://github.com/NIK-TIGER-BILL/MatryoshkaClaw/blob/main/Dockerfile)).
...
- **Optional:** In that `.env` you can set provider keys (e.g. `GROQ_API_KEY`, `OLLAMA_API_KEY`) and other MatryoshkaClaw env vars.
