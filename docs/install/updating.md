---
summary: "Updating MatryoshkaClaw safely (global install or source), plus rollback strategy"
read_when:
...
detects existing installs, upgrades in place, and runs `matryoshka doctor` when
needed.
...
- For **source installs**, use:
  ...
  ```bash
  curl -fsSL https://github.com/NIK-TIGER-BILL/MatryoshkaClaw/install.sh | bash -s -- --install-method git --no-onboard
  ```

  The installer will `git pull --rebase` **only** if the repo is clean.

- For **global installs**, the script uses `npm install -g matryoshkaclaw@latest` under the hood.
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current MatryoshkaClaw services.
...
- Windows (WSL2): `systemctl --user restart openclaw-gateway[-<profile>].service`
  - `launchctl`/`systemctl` only work if the service is installed; otherwise run `matryoshka gateway install`.
...
- Run `matryoshka doctor` again and read the output carefully (it often tells you the fix).
