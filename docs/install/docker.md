---
summary: "Optional Docker-based setup and onboarding for MatryoshkaClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run MatryoshkaClaw on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full MatryoshkaClaw in Docker)
...
- `org.opencontainers.image.source=https://github.com/NIK-TIGER-BILL/MatryoshkaClaw`
- `org.opencontainers.image.url=https://github.com/NIK-TIGER-BILL/MatryoshkaClaw`
- `org.opencontainers.image.documentation=https://docs.github.com/NIK-TIGER-BILL/MatryoshkaClaw/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=MatryoshkaClaw`
- `org.opencontainers.image.description=MatryoshkaClaw gateway and CLI runtime container image`
...
See [`ClawDock` Helper README](https://github.com/NIK-TIGER-BILL/MatryoshkaClaw/blob/main/scripts/shell-helpers/README.md) for details.
...
background. In plain terms: Docker keeps checking if MatryoshkaClaw is still
...
- noVNC observer access is password-protected by default; MatryoshkaClaw provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/NIK-TIGER-BILL/MatryoshkaClaw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: MatryoshkaClaw runs commands with `sh -lc` (login shell), which
