...
Deploy MatryoshkaClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/NIK-TIGER-BILL/MatryoshkaClaw)
...
repository configures everything needed to run MatryoshkaClaw:
...
If you use the original MatryoshkaClaw repository, Render will not auto-deploy your MatryoshkaClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For MatryoshkaClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any MatryoshkaClaw host.
