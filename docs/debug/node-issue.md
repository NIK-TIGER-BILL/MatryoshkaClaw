...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in MatryoshkaClaw
...
Running MatryoshkaClaw via Node with `tsx` fails at startup with:

```
[matryoshka] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `matryoshka status` and `gateway:watch` worked.
