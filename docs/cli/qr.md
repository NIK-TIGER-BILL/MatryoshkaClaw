---
summary: "CLI reference for `matryoshka qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
matryoshka qr
matryoshka qr --setup-code-only
matryoshka qr --json
matryoshka qr --remote
matryoshka qr --url wss://gateway.example/ws --token '<token>'
```
