---
summary: "CLI reference for `matryoshka configure` (interactive configuration prompts)"
read_when:
...
Tip: `matryoshka config` without a subcommand opens the same wizard. Use
`matryoshka config get|set|unset` for non-interactive edits.
...
```bash
matryoshka configure
matryoshka configure --section model --section channels
```
