---
summary: "CLI reference for `matryoshka onboard` (interactive onboarding wizard)"
read_when:
...
```bash
matryoshka onboard
matryoshka onboard --flow quickstart
matryoshka onboard --flow manual
matryoshka onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- Fastest first chat: `matryoshka dashboard` (Control UI, no channel setup).
