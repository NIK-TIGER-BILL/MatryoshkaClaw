...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a MatryoshkaClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`matryoshka acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the MatryoshkaClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
matryoshka acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
matryoshka acp --url wss://gateway-host:18789 --token-file ~/.openclaw/gateway.token

# Attach to an existing session key
matryoshka acp --session agent:main:main

# Attach by label (must already exist)
matryoshka acp --session-label "support inbox"

# Reset the session key before the first prompt
matryoshka acp --session agent:main:main --reset-session
```
...
```bash
matryoshka acp client

# Point the spawned bridge at a remote Gateway
matryoshka acp client --server-args --url wss://gateway-host:18789 --token-file ~/.openclaw/gateway.token

# Override the server command (default: matryoshka)
matryoshka acp client --server "node" --server-args openclaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a MatryoshkaClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `matryoshka acp` over stdio.

Example config (persisted):

```bash
matryoshka config set gateway.remote.url wss://gateway-host:18789
matryoshka config set gateway.remote.token <token>
```
...
```bash
matryoshka acp --session agent:main:main
matryoshka acp --session agent:design:main
matryoshka acp --session agent:qa:bug-123
```
...
MatryoshkaClaw bot over ACP, use `acpx` with its built-in `matryoshka` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx matryoshka` at `matryoshka acp`.
3. Target the MatryoshkaClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default MatryoshkaClaw ACP session
acpx matryoshka exec "Summarize the active MatryoshkaClaw session state."

# Persistent named session for follow-up turns
acpx matryoshka sessions ensure --name codex-bridge
acpx matryoshka -s codex-bridge --cwd /path/to/repo \
  "Ask my MatryoshkaClaw work agent for recent context relevant to this repo."
```

If you want `acpx matryoshka` to target a specific Gateway and session key every
time, override the `matryoshka` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "matryoshka": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 matryoshka acp --url ws://127.0.0.1:18789 --token-file ~/.openclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local MatryoshkaClaw checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an MatryoshkaClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “MatryoshkaClaw ACP” to start a thread.
...
- `matryoshka acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `matryoshka`).
