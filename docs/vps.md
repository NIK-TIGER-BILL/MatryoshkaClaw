---
summary: "VPS hosting hub for MatryoshkaClaw (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/matryoshka-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/matryoshka-compile-cache`
