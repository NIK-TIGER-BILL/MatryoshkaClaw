...
summary: "Complete reference for every MatryoshkaClaw config key, defaults, and channel settings"
read_when:
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — MatryoshkaClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `matryoshka doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `matryoshka doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["matryoshka-dm"] },
      guilds: {
        "123456789012345678": {
...
- MatryoshkaClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `matryoshka plugins install @matryoshka/mattermost`.
...
- `commands.callbackUrl` must resolve to the MatryoshkaClaw gateway endpoint and be reachable from the Mattermost server.
...
MatryoshkaClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `matryoshka channels add` (or channel onboarding) while still on a single-account top-level channel config, MatryoshkaClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `matryoshka doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@matryoshka"] },
      },
    ],
  },
}
...
Optional repository root shown in the system prompt's Runtime line. If unset, MatryoshkaClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, MatryoshkaClaw assumes `anthropic` (deprecated).
...
noVNC observer access uses VNC auth by default and MatryoshkaClaw emits a short-lived token URL (instead of exposing the password in the shared URL).
    ...
    list: [
      {
        ...
        groupChat: { mentionPatterns: ["@matryoshka"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
For `type: "acp"` entries, MatryoshkaClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, MatryoshkaClaw starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, MatryoshkaClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:matryoshka`   | All built-in tools (excludes provider plugins)                                           |
...
MatryoshkaClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openclaw/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), MatryoshkaClaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `matryoshka onboard --auth-choice opencode-zen` or `matryoshka onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `matryoshka onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `matryoshka onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `matryoshka onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `matryoshka onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `matryoshka onboard --auth-choice minimax-api`.
...
- `plugins.installs`: CLI-managed install metadata used by `matryoshka plugins update`.
...
Auth: `Authorization: Bearer <token>` or `x-matryoshka-token: <token>`.
...
- Hostname defaults to `matryoshka`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Setup: `matryoshka dns setup --apply`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one MatryoshkaClaw.`).
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `matryoshka doctor --fix` can strip unknown keys).
