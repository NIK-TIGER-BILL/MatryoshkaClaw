...
> MatryoshkaClaw is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
MatryoshkaClaw security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `matryoshka security audit`
...
```bash
matryoshka security audit
matryoshka security audit --deep
matryoshka security audit --fix
matryoshka security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

MatryoshkaClaw is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
MatryoshkaClaw assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.openclaw`, including `openclaw.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- MatryoshkaClaw can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want MatryoshkaClaw, use one VPS/host per user.
...
MatryoshkaClaw is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
If you run `--deep`, MatryoshkaClaw also attempts a best-effort live Gateway probe.
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full MatryoshkaClaw state                                 | filesystem perms on `~/.openclaw`                                                                 | yes      |
...
`matryoshka security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`matryoshka security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in MatryoshkaClaw config
schema:
...
- MatryoshkaClaw gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from MatryoshkaClaw responses.
...
MatryoshkaClaw stores session transcripts on disk under `~/.openclaw/agents/<agentId>/sessions/*.jsonl`.
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If MatryoshkaClaw cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
- If you don’t want remote execution, set security to **deny** and remove node pairing for that Mac.

## Dynamic skills (watcher / remote nodes)

MatryoshkaClaw can refresh the skills list mid-session:
...
- If you install plugins from npm (`matryoshka plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openclaw/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
  - MatryoshkaClaw uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
matryoshka pairing list <channel>
matryoshka pairing approve <channel> <code>
```
...
By default, MatryoshkaClaw routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
MatryoshkaClaw has two separate “who can trigger me?” layers:
...
MatryoshkaClaw includes explicit bypass flags that disable external-content safety wrapping:
...
`matryoshka doctor` can warn and offer to tighten these permissions.
...
If you run MatryoshkaClaw with Docker on a VPS, remember that published container ports
...
Doctor can generate one for you: `matryoshka doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), MatryoshkaClaw
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. MatryoshkaClaw verifies the identity by resolving the
...
- MatryoshkaClaw will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- When sharing diagnostics, prefer `matryoshka status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@matryoshka", "@mybot"] }
      }
    ]
  }
}
...
- Prefer a dedicated profile for the agent (the default `matryoshka` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only MatryoshkaClaw clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

MatryoshkaClaw’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default MatryoshkaClaw limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `matryoshka gateway` process.
...
4. Re-run `matryoshka security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + MatryoshkaClaw version
...
Found a vulnerability in MatryoshkaClaw? Please report responsibly:
