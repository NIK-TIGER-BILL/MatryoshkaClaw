...
MatryoshkaClaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. MatryoshkaClaw requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. MatryoshkaClaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
MatryoshkaClaw only treats a request as Serve when it arrives from loopback with
...
- Set `gateway.tailscale.resetOnExit` if you want MatryoshkaClaw to undo `tailscale serve`
