...
`matryoshka doctor` is the repair + migration tool for MatryoshkaClaw. It fixes stale
...
```bash
matryoshka doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
matryoshka doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
matryoshka doctor --repair --force
```
...
- Extra workspace dir detection (`~/matryoshka`).
...
migrated via `matryoshka doctor`.
...
offers to remove them and install the MatryoshkaClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named MatryoshkaClaw gateway services are considered first-class and are not
flagged as "extra."
...
- `matryoshka doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `matryoshka doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `matryoshka doctor` prompts before rewriting supervisor config.
- `matryoshka doctor --yes` accepts the default repair prompts.
- `matryoshka doctor --repair` applies recommended fixes without prompts.
- `matryoshka doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `matryoshka gateway install --force`.
