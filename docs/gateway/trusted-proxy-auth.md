...
read_when:
  - Running MatryoshkaClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of MatryoshkaClaw
...
- You run MatryoshkaClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. MatryoshkaClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. MatryoshkaClaw extracts the user identity from the configured header
...
- MatryoshkaClaw can stay on loopback HTTP behind the proxy.
...
If MatryoshkaClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://matryoshka.example.com
...
```
matryoshka.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy matryoshka:18789 {
...
location / {
    ...
    proxy_pass http://matryoshka:18789;
...
`matryoshka security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update MatryoshkaClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `matryoshka security audit` and review findings
