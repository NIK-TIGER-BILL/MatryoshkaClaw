...
MatryoshkaClaw supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `matryoshka gateway`).
...
API keys for daemon use: `matryoshka onboard`.
...
Then paste it into MatryoshkaClaw:
...
```bash
matryoshka models auth paste-token --provider anthropic
matryoshka models auth paste-token --provider openrouter
```
...
```bash
matryoshka models status --check
```
...
- MatryoshkaClaw retries with the next key only for rate-limit errors (for example
...
Run `matryoshka models status` to confirm which profile is expiring. If the profile
