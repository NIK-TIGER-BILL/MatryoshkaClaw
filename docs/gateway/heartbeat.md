...
- During heartbeat runs, MatryoshkaClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `matryoshka sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, MatryoshkaClaw skips the heartbeat run entirely (no model call).
...
```bash
matryoshka system event --text "Check for urgent follow-ups" --mode now
```
