...
read_when:
  - Setting up MatryoshkaClaw for the first time
...
MatryoshkaClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.openclaw/openclaw.json`.

If the file is missing, MatryoshkaClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `matryoshka onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    matryoshka onboard       # full setup wizard
    matryoshka configure     # config wizard
    ```
  </Tab>
...
<Warning>
MatryoshkaClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`matryoshka doctor`, `matryoshka logs`, `matryoshka health`, `matryoshka status`)
- Run `matryoshka doctor` to see exact issues
- Run `matryoshka doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `matryoshka config set` for single keys.
    </Warning>
...
MatryoshkaClaw reads env vars from the parent process plus:
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, MatryoshkaClaw runs your login shell and imports only the missing keys:
