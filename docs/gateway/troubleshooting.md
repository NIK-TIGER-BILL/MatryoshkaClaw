...
- `matryoshka gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `matryoshka doctor` reports no blocking config/service issues.
- `matryoshka channels status --probe` shows connected/ready channels.
...
matryoshka config get agents.defaults.models
```
...
matryoshka status
matryoshka channels status --probe
matryoshka pairing list --channel <channel> [--account <id>]
matryoshka config get channels
matryoshka logs --follow
```
...
matryoshka logs --follow
matryoshka doctor
matryoshka gateway status --json
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `matryoshka config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `matryoshka devices list` then `matryoshka devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `matryoshka configure`). If you are running MatryoshkaClaw via Podman using the dedicated `matryoshka` user, the config lives at `~openclaw/.openclaw/openclaw.json`.
...
```bash
matryoshka cron status
matryoshka cron list
matryoshka cron runs --id <jobId> --limit 20
matryoshka system heartbeat last
matryoshka logs --follow
```
...
matryoshka approvals get --node <idOrNameOrIp>
matryoshka logs --follow
matryoshka status
```
...
matryoshka browser start --browser-profile matryoshka
...
```bash
matryoshka gateway status
matryoshka config get gateway.mode
matryoshka config get gateway.remote.url
matryoshka config get gateway.auth.mode
```
...
```bash
matryoshka config get gateway.bind
