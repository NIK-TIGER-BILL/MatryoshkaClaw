...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/matryoshka`)
...
If the page fails to load, start the Gateway first: `matryoshka gateway`.
...
you revoke it with `matryoshka devices revoke --device <id> --role <role>`. See
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop matryoshka`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. MatryoshkaClaw
...
MatryoshkaClaw **blocks** Control UI connections without device identity.
