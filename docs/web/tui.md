...
```bash
matryoshka tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `matryoshka tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `matryoshka tui`.
...
- Check the Gateway logs: `matryoshka logs --follow`.
- Confirm the agent can run: `matryoshka status` and `matryoshka models status`.
...
- No agents in picker: check `matryoshka agents list` and your routing config.
