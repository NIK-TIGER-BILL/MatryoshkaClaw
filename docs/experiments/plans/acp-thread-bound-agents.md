...
This plan defines how MatryoshkaClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- MatryoshkaClaw core owns ACP control plane concerns
...
MatryoshkaClaw should not reimplement ACP transport internals in core.
MatryoshkaClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in MatryoshkaClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under MatryoshkaClaw state dir
...
Core (must be in MatryoshkaClaw core):
...
- ACP event stream is normalized to MatryoshkaClaw reply chunks
...
For the first production backend (`extensions/acpx`), MatryoshkaClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses MatryoshkaClaw `sessionKey`)
...
- MatryoshkaClaw consumes ndjson events from `acpx --format json --json-strict`
