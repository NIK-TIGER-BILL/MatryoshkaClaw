...
read_when:
  ...
  - Deciding: standalone CLI vs deep MatryoshkaClaw integration
...
Recommendation: **deep integration in MatryoshkaClaw**, but keep a separable core library.

### Why integrate into MatryoshkaClaw?

- MatryoshkaClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `matryoshka memory recall "…" --k 25 --since 30d`
  - `matryoshka memory reflect --since 7d`
