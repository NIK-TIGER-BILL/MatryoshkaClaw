...
summary: "Architecture of MatryoshkaClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in MatryoshkaClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how MatryoshkaClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

MatryoshkaClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, MatryoshkaClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in MatryoshkaClaw's local TUI mode)                                             |
...
├── matryoshka-tools.ts              # MatryoshkaClaw-specific tools
...
Image injection is prompt-local: MatryoshkaClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: MatryoshkaClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **MatryoshkaClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures MatryoshkaClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, MatryoshkaClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
MatryoshkaClaw wraps this with `guardSessionManager()` for tool result safety.
...
MatryoshkaClaw maintains an auth profile store with multiple API keys per provider:
...
MatryoshkaClaw loads custom pi extensions for specialized behavior:
...
MatryoshkaClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | MatryoshkaClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom MatryoshkaClaw tool suite                                                                     |
