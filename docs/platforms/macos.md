---
summary: "MatryoshkaClaw macOS companion app (menu bar + gateway broker)"
read_when:
...
# MatryoshkaClaw macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for MatryoshkaClaw. It owns permissions,
...
- Installs the global CLI (`matryoshka`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `matryoshka gateway install`.
...
The app registers the `matryoshka://` URL scheme for local actions.
...
```bash
open 'matryoshka://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **MatryoshkaClaw.app**.
...
Avoid putting your MatryoshkaClaw state dir in iCloud or other cloud-synced folders.
...
If `matryoshka doctor` detects state under:
...
- `swift run MatryoshkaClaw` (or Xcode)
...
```bash
cd apps/macos
swift run matryoshka-mac connect --json
swift run matryoshka-mac discover --timeout 3000 --json
```
...
Tip: compare against `matryoshka gateway discover --json` to see whether the
