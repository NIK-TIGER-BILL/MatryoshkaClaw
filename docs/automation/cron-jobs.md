...
- For upgrades, `matryoshka doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
matryoshka cron run <job-id>
matryoshka cron runs --id <job-id>
```
...
are only safe when the Gateway is stopped. Prefer `matryoshka cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, MatryoshkaClaw applies a
...
If `delivery` is omitted for isolated jobs, MatryoshkaClaw defaults to `announce`.
...
- CLI equivalents: `matryoshka cron add --light-context ...` and `matryoshka cron edit --light-context`.
...
When a job fails, MatryoshkaClaw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, MatryoshkaClaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `matryoshka cron runs` and adjust retention before logs become large
...
matryoshka cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
matryoshka cron edit <jobId> --agent ops
matryoshka cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
matryoshka cron run <jobId>
matryoshka cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `matryoshka cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
matryoshka cron edit <jobId> \
...
```bash
matryoshka cron edit <jobId> --exact
```
...
```bash
matryoshka system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`matryoshka system event`](/cli/system).
...
- MatryoshkaClaw applies exponential retry backoff for recurring jobs after consecutive errors:
