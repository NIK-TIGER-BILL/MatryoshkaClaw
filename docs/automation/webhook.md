...
read_when:
  ...
  - Wiring external systems into MatryoshkaClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your MatryoshkaClaw config directory (typically `~/.openclaw/hooks/transforms`).
...
- `matryoshka webhooks gmail setup` writes `hooks.gmail` config for `matryoshka webhooks gmail run`.
