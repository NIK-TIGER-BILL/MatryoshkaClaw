---
summary: "Gmail Pub/Sub push wired into MatryoshkaClaw webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to MatryoshkaClaw
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> MatryoshkaClaw

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> MatryoshkaClaw webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- MatryoshkaClaw hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the MatryoshkaClaw helper to wire everything together (installs deps on macOS via brew):

```bash
matryoshka webhooks gmail setup \
...
- Writes `hooks.gmail` config for `matryoshka webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, MatryoshkaClaw automatically sets
...
- `--hook-url` points to MatryoshkaClaw `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to MatryoshkaClaw.

Recommended: `matryoshka webhooks gmail run` wraps the same flow and auto-renews the watch.
