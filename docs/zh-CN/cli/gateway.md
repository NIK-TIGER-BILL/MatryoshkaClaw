...
summary: MatryoshkaClaw Gateway 网关 CLI（`matryoshka gateway`）— 运行、查询和发现 Gateway 网关
title: gateway
x-i18n:
...
Gateway 网关是 MatryoshkaClaw 的 WebSocket 服务器（渠道、节点、会话、hooks）。

本页中的子命令位于 `matryoshka gateway …` 下。
