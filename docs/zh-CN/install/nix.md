...
read_when:
  ...
  - 你想要所有内容都固定并以声明式管理
summary: 使用 Nix 声明式安装 MatryoshkaClaw
title: Nix
x-i18n:
...
使用 Nix 运行 MatryoshkaClaw 的推荐方式是通过 **[nix-openclaw](https://github.com/matryoshka/nix-openclaw)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/matryoshka/nix-openclaw](https://github.com/matryoshka/nix-openclaw)**
>
...
MatryoshkaClaw 支持 **Nix 模式**，使配置确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
```bash
defaults write bot.molt.mac matryoshka.nixMode -bool true
```

### 配置 + 状态路径

MatryoshkaClaw 从 `OPENCLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `OPENCLAW_STATE_DIR` 中。
...
```
apps/macos/Sources/MatryoshkaClaw/Resources/Info.plist
```
...
- [nix-openclaw](https://github.com/matryoshka/nix-openclaw) — 完整设置指南
