---
read_when:
  - 你正在将 MatryoshkaClaw 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 MatryoshkaClaw 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
# 将 MatryoshkaClaw 迁移到新机器

本指南将 MatryoshkaClaw Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。
...
- `matryoshka status` 显示 Gateway 网关正在运行
...
- [MatryoshkaClaw 在哪里存储数据？](/help/faq#where-does-matryoshka-store-its-data)
