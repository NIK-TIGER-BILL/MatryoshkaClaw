...
最快聊天：打开 Control UI（无需渠道设置）。运行 `matryoshka dashboard` 并在浏览器中聊天，或在 Gateway 网关主机上打开 `http://127.0.0.1:18789/`。文档：[Dashboard](/web/dashboard) 和 [Control UI](/web/control-ui)。

推荐路径：使用 **CLI 新手引导向导**（`matryoshka onboard`）。它设置：
...
- Node `>=22`
- `pnpm`（可选；如果从源代码构建则推荐）
- **推荐：**Brave Search API 密钥用于网页搜索。最简单的方式：`matryoshka configure --section web`（存储 `tools.web.search.apiKey`）。参见 [Web 工具](/tools/web)。
...
如果你正在开发 MatryoshkaClaw 本身，从源代码运行：
...
如果你还没有全局安装，从仓库通过 `pnpm matryoshka ...` 运行新手引导步骤。`pnpm build` 也会打包 A2UI 资源；如果你只需要运行那个步骤，使用 `pnpm canvas:a2ui:bundle`。
...
```bash
matryoshka message send --target +15555550123 --message "Hello from MatryoshkaClaw"
```

如果 `matryoshka health` 显示"未配置认证"，回到向导设置 OAuth/密钥认证——没有它智能体将无法响应。

提示：`matryoshka status --all` 是最佳的可粘贴、只读调试报告。
健康探测：`matryoshka health`（或 `matryoshka status --deep`）向运行中的 Gateway 网关请求健康快照。
