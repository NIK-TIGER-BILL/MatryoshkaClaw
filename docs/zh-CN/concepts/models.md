...
- `agents.defaults.models` 是 MatryoshkaClaw 可使用的模型白名单/目录（加上别名）。
...
如果设置了 `agents.defaults.models`，它将成为 `/model` 和会话覆盖的**白名单**。当用户选择不在该白名单中的模型时，MatryoshkaClaw 返回：
...
- 如果省略提供商，MatryoshkaClaw 将输入视为别名或**默认提供商**的模型（仅在模型 ID 中没有 `/` 时有效）。
...
`matryoshka models`（无子命令）是 `models status` 的快捷方式。
...
`matryoshka models scan` 检查 OpenRouter 的**免费模型目录**，并可选择性地探测模型的工具和图像支持。
