---
read_when:
  - 为 MatryoshkaClaw 设置 Zalo Personal
...
- 通过 CLI 安装：`matryoshka plugins install @matryoshka/zalouser`
- 或从源码检出安装：`matryoshka plugins install ./extensions/zalouser`
- 详情：[插件](/tools/plugin)
...
- 配置向导可以提示输入群组允许列表。
- 启动时，MatryoshkaClaw 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`matryoshka channels logout --channel zalouser && matryoshka channels login --channel zalouser`
