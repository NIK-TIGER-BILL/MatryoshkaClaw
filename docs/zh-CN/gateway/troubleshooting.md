...
read_when:
  - 调查运行时问题或故障
summary: MatryoshkaClaw 常见故障的快速故障排除指南
title: 故障排除
x-i18n:
...
当 MatryoshkaClaw 出现异常时，以下是解决方法。
...
| 命令                               | 它告诉你什么                                                                          | 何时使用                              |
| ---------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------- |
| `matryoshka status`                  | 本地摘要：操作系统 + 更新、Gateway 网关可达性/模式、服务、智能体/会话、提供商配置状态 | 首次检查，快速概览                    |
| `matryoshka status --all`            | 完整本地诊断（只读、可粘贴、相对安全）包括日志尾部                                    | 当你需要分享调试报告时                |
| `matryoshka status --deep`           | 运行 Gateway 网关健康检查（包括提供商探测；需要可达的 Gateway 网关）                  | 当"已配置"不意味着"正常工作"时        |
| `matryoshka gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `matryoshka channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
| `matryoshka gateway status`          | 监管程序状态（launchd/systemd/schtasks）、运行时 PID/退出、最后的 Gateway 网关错误    | 当服务"看起来已加载"但没有运行时      |
| `matryoshka logs --follow`           | 实时日志（运行时问题的最佳信号）                                                      | 当你需要实际的故障原因时              |

**分享输出：** 优先使用 `matryoshka status --all`（它会隐藏令牌）。如果你粘贴 `matryoshka status`，考虑先设置 `OPENCLAW_SHOW_SECRETS=0`（令牌预览）。
...
```bash
matryoshka models auth paste-token --provider anthropic
matryoshka models status
```
...
- 或直接设置：
  ```bash
  matryoshka config set gateway.mode local
  ```
...
- 设置远程 URL 并保持 `gateway.mode=remote`：
  ```bash
  matryoshka config set gateway.mode remote
  matryoshka config set gateway.remote.url "wss://gateway.example.com"
  ```

**仅临时/开发使用：** 传递 `--allow-unconfigured` 以在没有
`gateway.mode=local` 的情况下启动 Gateway 网关。

**还没有配置文件？** 运行 `matryoshka setup` 创建初始配置，然后重新运行
Gateway 网关。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `matryoshka doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `matryoshka sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `matryoshka gateway` 和服务。
- 如果你设置了 `gateway.mode=remote`，**CLI 默认**使用远程 URL。服务可能仍在本地运行，但你的 CLI 可能在探测错误的位置。使用 `matryoshka gateway status` 查看服务解析的端口 + 探测目标（或传递 `--url`）。
- `matryoshka gateway status` 和 `matryoshka doctor` 在服务看起来正在运行但端口关闭时会显示日志中的**最后 Gateway 网关错误**。
...
**如果 `matryoshka gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该匹配。
- 如果不匹配，你几乎肯定是在编辑一个配置而服务运行的是另一个。
- 修复：从你希望服务使用的相同 `--profile` / `OPENCLAW_STATE_DIR` 重新运行 `matryoshka gateway install --force`。

**如果 `matryoshka gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `matryoshka doctor` 更新它（或 `matryoshka gateway install --force` 完全重写）。
...
**如果 `matryoshka gateway status` 显示 `bind=tailnet` 但未找到 tailnet 接口**
...
如果你从旧版本升级，你的磁盘上可能仍有 `~/matryoshka`。
...
MatryoshkaClaw 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。

**修复：**

- 为提供商选择**最新**模型并更新你的配置或模型别名。
- 如果你不确定哪些模型可用，运行 `matryoshka models list` 或
  `matryoshka models scan` 并选择一个支持的模型。
...
```bash
matryoshka logs --follow | grep "pairing request"
```
...
# 查看最近的连接事件
matryoshka logs --limit 200 | grep "connection\\|disconnect\\|logout"
```

**修复：** 通常在 Gateway 网关运行后会自动重连。如果卡住，重启 Gateway 网关进程（无论你如何监管它），或使用详细输出手动运行：

```bash
matryoshka gateway --verbose
```

如果你已登出/取消关联：

```bash
matryoshka channels logout
trash "${OPENCLAW_STATE_DIR:-$HOME/.openclaw}/credentials" # 如果 logout 无法完全清除所有内容
matryoshka channels login --verbose       # 重新扫描二维码
```
...
当配置包含未知键、格式错误的值或无效类型时，MatryoshkaClaw 现在拒绝启动。
这是为了安全而故意设计的。
...
```bash
matryoshka doctor
matryoshka doctor --fix
```

注意事项：

- `matryoshka doctor` 报告每个无效条目。
- `matryoshka doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `matryoshka logs`、`matryoshka health`、`matryoshka status`、`matryoshka gateway status` 和 `matryoshka gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/matryoshka/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `matryoshka models status`（CLI）。
...
5. 运行 `matryoshka channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。MatryoshkaClaw 在当前 `main` 中清理/规范化工具
...
**修复 3：检查 CLI 安装**
确保全局 `matryoshka` CLI 已安装且与应用版本匹配：
...
matryoshka gateway --verbose
matryoshka channels login --verbose
```
...
| Gateway 网关服务日志（监管程序） | macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.openclaw/logs/...`；配置文件使用 `~/.openclaw-<profile>/logs/...`）<br />Linux：`journalctl --user -u openclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "MatryoshkaClaw Gateway (<profile>)" /V /FO LIST` |
...
matryoshka gateway status --deep

# Gateway 网关是否可达？
matryoshka health --json
# 如果失败，使用连接详情重新运行：
matryoshka health --verbose
...
matryoshka channels login         # 重新配对 WhatsApp
matryoshka gateway restart           # 或：matryoshka gateway
```
...
1. 首先检查日志：`/tmp/matryoshka/`（默认：`matryoshka-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - MatryoshkaClaw 版本
   - 相关日志片段
