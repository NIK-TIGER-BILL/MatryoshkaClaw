...
MatryoshkaClaw 从 `~/.openclaw/openclaw.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，MatryoshkaClaw 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.openclaw/workspace`）。通常只在以下情况需要配置：
...
MatryoshkaClaw 只接受完全匹配 schema 的配置。
未知键、类型错误或无效值会导致 Gateway 网关 **拒绝启动**以确保安全。

验证失败时：

- Gateway 网关不会启动。
- 只允许诊断命令（例如：`matryoshka doctor`、`matryoshka logs`、`matryoshka health`、`matryoshka status`、`matryoshka service`、`matryoshka help`）。
- 运行 `matryoshka doctor` 查看具体问题。
- 运行 `matryoshka doctor --fix`（或 `--yes`）应用迁移/修复。
...
请使用 `config.patch` 或 `matryoshka config set`。请备份 `~/.openclaw/openclaw.json`。
...
```bash
matryoshka gateway call config.get --params '{}' # capture payload.hash
matryoshka gateway call config.apply --params '{
...
```bash
matryoshka gateway call config.get --params '{}' # capture payload.hash
matryoshka gateway call config.patch --params '{
...
```json5
{
  agents: {
    ...
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@matryoshka", "reisponde"] },
      },
    ],
  },
  channels: {
...
{ "$include": "/etc/matryoshka/base.json5" } // 绝对路径
...
MatryoshkaClaw 从父进程（shell、launchd/systemd、CI 等）读取环境变量。
...
可选便利功能：如果启用且预期键均未设置，MatryoshkaClaw 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
MatryoshkaClaw 在以下位置存储**每个智能体的**认证配置文件（OAuth + API 密钥）：
...
- `~/.openclaw/agent/*`（由 `matryoshka doctor` 迁移到 `~/.openclaw/agents/<defaultAgentId>/agent/*`）
...
首次使用时，MatryoshkaClaw 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
如果设置了，MatryoshkaClaw 会推导默认值（仅在你未明确设置时）：
...
- 如需稳定路径，将 `logging.file` 设为 `/tmp/matryoshka/openclaw.log`。
- 控制台输出可通过以下方式单独调整：
...
- 旧版单账号 Baileys 认证目录由 `matryoshka doctor` 迁移到 `whatsapp/default`。
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@matryoshka", "matryoshka"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          // 仅这些文本模式会触发响应
          mentionPatterns: ["reisponde", "@matryoshka"],
        },
      },
    ],
  },
...
MatryoshkaClaw 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["matryoshka-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
...
MatryoshkaClaw 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
MatryoshkaClaw 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
请先安装：`matryoshka plugins install @matryoshka/mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
MatryoshkaClaw 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
MatryoshkaClaw 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，MatryoshkaClaw 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/matryoshka" } },
}
```
...
当文件超过此限制时，MatryoshkaClaw 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，MatryoshkaClaw 在运行时使用主机时区。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[matryoshka]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[matryoshka]"`，否则为 `""`（无前缀）。使用 `"[matryoshka]"` 时，如果路由的智能体设置了 `identity.name`，MatryoshkaClaw 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，MatryoshkaClaw 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
MatryoshkaClaw 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
如果省略提供商，MatryoshkaClaw 目前假定 `anthropic` 作为临时弃用回退。
...
- `tools.web.search.apiKey`（推荐：通过 `matryoshka configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
- `group:nodes`：`nodes`
- `group:matryoshka`：所有内置 MatryoshkaClaw 工具（不包含提供商插件）
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          containerPrefix: "matryoshka-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          ...
          containerPrefix: "matryoshka-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
MatryoshkaClaw 使用 **pi-coding-agent** 模型目录。你可以通过编写
`~/.openclaw/agents/<agentId>/agent/models.json` 或在 MatryoshkaClaw 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，MatryoshkaClaw 在启动时将 `models.json` 写入/合并到
...
OpenCode Zen 是一个具有每模型端点的多模型网关。MatryoshkaClaw 使用
...
- 快捷方式：`matryoshka onboard --auth-choice opencode-zen`。
...
快捷方式：`matryoshka onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `matryoshka onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `matryoshka onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `matryoshka onboard --auth-choice synthetic-api-key`。
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `matryoshka onboard --auth-choice minimax-api`。
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，MatryoshkaClaw 保持仅空闲模式以向后兼容。
...
### `browser`（MatryoshkaClaw 管理的浏览器）

MatryoshkaClaw 可以为 MatryoshkaClaw 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（MatryoshkaClaw.app 菜单栏或 `matryoshka gateway`）启动。
...
- 示例：`"/ui"`、`"/matryoshka"`、`"/apps/matryoshka"`。
- 默认：根路径（`/`）（不变）。
...
- 当连接来自这些 IP 之一时，MatryoshkaClaw 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
- 仅列出你完全控制的代理，并确保它们**覆盖**传入的 `x-forwarded-for`。

说明：

- `matryoshka gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- `gateway.auth.allowTailscale` 允许 Tailscale Serve 身份头部
  ...
  `x-forwarded-proto` 和 `x-forwarded-host` 时满足认证。MatryoshkaClaw 在接受之前
...
- MatryoshkaClaw.app 监视 `~/.openclaw/openclaw.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
- `matryoshka --dev …` → 使用 `~/.openclaw-dev` + 端口从基础 `19001` 偏移
- `matryoshka --profile <name> …` → 使用 `~/.openclaw-<name>`（端口通过配置/环境变量/标志）
...
Gmail 辅助配置（由 `matryoshka webhooks gmail setup` / `run` 使用）：
...
注意：当 `tailscale.mode` 开启时，MatryoshkaClaw 将 `serve.path` 默认为 `/`，以便
...
默认端口：`18793`（选择此端口以避免 MatryoshkaClaw 浏览器 CDP 端口 `18792`）
...
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `matryoshka`（通告 `matryoshka.local`）。通过 `OPENCLAW_MDNS_HOSTNAME` 覆盖。
