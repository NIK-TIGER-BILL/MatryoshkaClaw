...
- 替代旧版 `gateway` 命令。CLI 入口点：`matryoshka gateway`。
...
- Windows：`MatryoshkaClaw Gateway (<profile>)`

安装元数据嵌入在服务配置中：

- `OPENCLAW_SERVICE_MARKER=matryoshka`
...
```bash
matryoshka --dev setup
matryoshka --dev gateway --allow-unconfigured
# 然后定位到 dev 实例：
matryoshka --dev status
matryoshka --dev health
```
...
```bash
matryoshka --profile main gateway install
matryoshka --profile rescue gateway install
```

示例：

```bash
OPENCLAW_CONFIG_PATH=~/.openclaw/a.json OPENCLAW_STATE_DIR=~/.openclaw-a matryoshka gateway --port 19001
OPENCLAW_CONFIG_PATH=~/.openclaw/b.json OPENCLAW_STATE_DIR=~/.openclaw-b matryoshka gateway --port 19002
```
...
- `health` — 完整健康快照（与 `matryoshka health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，使用自定义 LaunchDaemon（未随附）。
  - `matryoshka gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.openclaw.*` 会被清理）。
  - `matryoshka doctor` 审计 LaunchAgent 配置，可以将其更新为当前默认值。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 MatryoshkaClaw 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`matryoshka gateway uninstall`（当前服务）和 `matryoshka doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `matryoshka gateway install --force` 重新安装（配置文件/env/路径更改）。

捆绑的 mac 应用：

- MatryoshkaClaw.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.openclaw.*` 标签仍能干净卸载）的按用户 LaunchAgent。
- 要干净地停止它，使用 `matryoshka gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `matryoshka gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `matryoshka gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 监管（systemd 用户单元）

MatryoshkaClaw 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`matryoshka gateway install` 写入用户单元。`matryoshka doctor` 审计
...
- `matryoshka gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `matryoshka message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `matryoshka agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `matryoshka gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `matryoshka gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `matryoshka gateway` 和旧版 TCP 控制端口的使用。
