...
```bash
matryoshka channels logs --channel whatsapp
```
...
- MatryoshkaClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `matryoshka plugins enable diagnostics-otel` 启用插件。
...
- `matryoshka.tokens`（计数器，属性：`matryoshka.token`、`matryoshka.channel`、`matryoshka.provider`、`matryoshka.model`）
- `matryoshka.cost.usd`（计数器，属性：`matryoshka.channel`、`matryoshka.provider`、`matryoshka.model`）
- `matryoshka.run.duration_ms`（直方图，属性：`matryoshka.channel`、`matryoshka.provider`、`matryoshka.model`）
- `matryoshka.context.tokens`（直方图，属性：`matryoshka.context`、`matryoshka.channel`、`matryoshka.provider`、`matryoshka.model`）

消息流：

- `matryoshka.webhook.received`（计数器，属性：`matryoshka.channel`、`matryoshka.webhook`）
- `matryoshka.webhook.error`（计数器，属性：`matryoshka.channel`、`matryoshka.webhook`）
- `matryoshka.webhook.duration_ms`（直方图，属性：`matryoshka.channel`、`matryoshka.webhook`）
- `matryoshka.message.queued`（计数器，属性：`matryoshka.channel`、`matryoshka.source`）
- `matryoshka.message.processed`（计数器，属性：`matryoshka.channel`、`matryoshka.outcome`）
- `matryoshka.message.duration_ms`（直方图，属性：`matryoshka.channel`、`matryoshka.outcome`）

队列 + 会话：

- `matryoshka.queue.lane.enqueue`（计数器，属性：`matryoshka.lane`）
- `matryoshka.queue.lane.dequeue`（计数器，属性：`matryoshka.lane`）
- `matryoshka.queue.depth`（直方图，属性：`matryoshka.lane` 或 `matryoshka.channel=heartbeat`）
- `matryoshka.queue.wait_ms`（直方图，属性：`matryoshka.lane`）
- `matryoshka.session.state`（计数器，属性：`matryoshka.state`、`matryoshka.reason`）
- `matryoshka.session.stuck`（计数器，属性：`matryoshka.state`）
- `matryoshka.session.stuck_age_ms`（直方图，属性：`matryoshka.state`）
- `matryoshka.run.attempt`（计数器，属性：`matryoshka.attempt`）

### 导出的 span（名称 + 关键属性）

- `matryoshka.model.usage`
  - `matryoshka.channel`、`matryoshka.provider`、`matryoshka.model`
  - `matryoshka.sessionKey`、`matryoshka.sessionId`
  - `matryoshka.tokens.*`（input/output/cache_read/cache_write/total）
- `matryoshka.webhook.processed`
  - `matryoshka.channel`、`matryoshka.webhook`、`matryoshka.chatId`
- `matryoshka.webhook.error`
  - `matryoshka.channel`、`matryoshka.webhook`、`matryoshka.chatId`、`matryoshka.error`
- `matryoshka.message.processed`
  - `matryoshka.channel`、`matryoshka.outcome`、`matryoshka.chatId`、`matryoshka.messageId`、`matryoshka.sessionKey`、`matryoshka.sessionId`、`matryoshka.reason`
- `matryoshka.session.stuck`
  - `matryoshka.state`、`matryoshka.ageMs`、`matryoshka.queueDepth`、`matryoshka.sessionKey`、`matryoshka.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `matryoshka doctor`。
