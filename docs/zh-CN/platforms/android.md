...
- 你可以在 Gateway 网关机器上运行 CLI（`matryoshka`）（或通过 SSH）。
...
```bash
matryoshka nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
