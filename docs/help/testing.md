...
MatryoshkaClaw has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  - MatryoshkaClaw uses Vitest `vmForks` on Node 22/23 for faster unit shards.
  - On Node 24+, MatryoshkaClaw automatically falls back to regular `forks` to avoid Node VM linking errors (`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`).
...
### Layer 2: Gateway + dev agent smoke (what “@matryoshka” actually does)
...
```bash
matryoshka models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: MatryoshkaClaw calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: MatryoshkaClaw shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `matryoshka models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `matryoshka models list` / model selection.
