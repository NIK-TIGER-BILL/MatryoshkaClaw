---
summary: "How MatryoshkaClaw builds prompt context and reports token usage + costs"
read_when:
...
MatryoshkaClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

MatryoshkaClaw assembles its own system prompt on every run. It includes:
...
For images, MatryoshkaClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from MatryoshkaClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `matryoshka status --usage` and `matryoshka channels list` show
...
`cacheWrite`. If pricing is missing, MatryoshkaClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. MatryoshkaClaw can
...
Anthropic's 1M context window is currently beta-gated. MatryoshkaClaw can inject the
...
MatryoshkaClaw skips the `context-1m-*` beta header because Anthropic currently
