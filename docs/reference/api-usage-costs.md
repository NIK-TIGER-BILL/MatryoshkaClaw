...
MatryoshkaClaw features that can generate provider usage or paid API calls.
...
- `matryoshka status --usage` and `matryoshka channels list` show provider **usage windows**
...
MatryoshkaClaw can pick up credentials from:
...
- `matryoshka status --usage`
- `matryoshka models status --json`
...
`matryoshka models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
