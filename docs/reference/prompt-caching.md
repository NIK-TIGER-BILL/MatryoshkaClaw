...
- With Anthropic API-key auth profiles, MatryoshkaClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, MatryoshkaClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
MatryoshkaClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
