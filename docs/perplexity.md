...
MatryoshkaClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, MatryoshkaClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `matryoshka configure --section web`. It stores the key in
...
`~/.openclaw/.env` (or your service environment). See [Env vars](/help/faq#how-does-matryoshka-load-environment-variables).
