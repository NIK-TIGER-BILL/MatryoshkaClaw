...
The onboarding wizard is the **recommended** way to set up MatryoshkaClaw on macOS,
...
`matryoshka dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`matryoshka configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
If the config is invalid or contains legacy keys, the wizard asks you to run `matryoshka doctor` first.
</Note>
...
Use `matryoshka agents add <name>` to create a separate agent with its own workspace,
...
- CLI command reference: [`matryoshka onboard`](/cli/onboard)
