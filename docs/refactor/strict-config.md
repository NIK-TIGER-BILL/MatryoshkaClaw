...
- Plugin manifests (`matryoshka.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`matryoshka.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `matryoshka doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `matryoshka doctor --fix`.”
