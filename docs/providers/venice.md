---
summary: "Use Venice AI privacy-focused models in MatryoshkaClaw"
read_when:
  - You want privacy-focused inference in MatryoshkaClaw
...
```bash
matryoshka onboard --auth-choice venice-api-key
```
...
```bash
matryoshka onboard --non-interactive \
...
```bash
matryoshka agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, MatryoshkaClaw shows all available Venice models. Pick based on your needs:
...
```bash
matryoshka models set venice/kimi-k2-5
matryoshka models set venice/claude-opus-4-6
```

List all available models:

```bash
matryoshka models list | grep venice
```

## Configure via `matryoshka configure`

1. Run `matryoshka configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
MatryoshkaClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
matryoshka agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
matryoshka agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
matryoshka agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
matryoshka agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
matryoshka agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `matryoshka models list` to see currently available models. Some models may be temporarily offline.
