---
summary: "Use OpenCode Zen and Go catalogs with MatryoshkaClaw"
read_when:
...
OpenCode exposes two hosted catalogs in MatryoshkaClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. MatryoshkaClaw keeps the runtime provider ids
...
matryoshka onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
matryoshka onboard --auth-choice opencode-go
matryoshka onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
