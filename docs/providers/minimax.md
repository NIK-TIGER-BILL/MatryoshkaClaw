---
summary: "Use MiniMax M2.5 in MatryoshkaClaw"
read_when:
  - You want MiniMax models in MatryoshkaClaw
...
```bash
matryoshka plugins enable minimax-portal-auth  # skip if already loaded.
matryoshka gateway restart  # restart if gateway is already running
matryoshka onboard --auth-choice minimax-portal
```
...
- Use `matryoshka models list` and `matryoshka models set minimax/MiniMax-M2.5` to switch.
...
- Running `matryoshka configure` and selecting **MiniMax M2.5**, or
