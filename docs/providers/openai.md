---
summary: "Use OpenAI via API keys or Codex subscription in MatryoshkaClaw"
read_when:
  - You want to use OpenAI models in MatryoshkaClaw
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like MatryoshkaClaw.
...
```bash
matryoshka onboard --auth-choice openai-api-key
# or non-interactive
matryoshka onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. MatryoshkaClaw forwards both through the `openai/*` Responses path.
...
matryoshka onboard --auth-choice openai-codex

# Or run OAuth directly
matryoshka models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. MatryoshkaClaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

MatryoshkaClaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), MatryoshkaClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. MatryoshkaClaw enables it by default for
...
MatryoshkaClaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), MatryoshkaClaw now auto-enables OpenAI server-side
