...
read_when:
  - Setting up Twitch chat integration for MatryoshkaClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. MatryoshkaClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
matryoshka plugins install @matryoshka/twitch
```

Local checkout (when running from a git repo):

```bash
matryoshka plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "matryoshka", // Bot's Twitch account
