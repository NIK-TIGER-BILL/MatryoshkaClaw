...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects MatryoshkaClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current MatryoshkaClaw releases, so no separate plugin install
is required.
...
```bash
matryoshka plugins install @matryoshka/feishu
```
...
If you just installed MatryoshkaClaw, run the wizard:
...
2. Configuring app credentials in MatryoshkaClaw
...
1. You already ran `matryoshka channels add` for Feishu
2. The gateway is running (`matryoshka gateway status`)
...
## Step 2: Configure MatryoshkaClaw
...
```bash
matryoshka pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  matryoshka pairing list feishu
  matryoshka pairing approve feishu <CODE>
  ```
...
2. Run `matryoshka logs --follow` and look for `chat_id`
...
2. Run `matryoshka logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `matryoshka gateway status`  | Show gateway status           |
| `matryoshka gateway install` | Install/start gateway service |
| `matryoshka gateway stop`    | Stop gateway service          |
| `matryoshka gateway restart` | Restart gateway service       |
| `matryoshka logs --follow`   | Tail gateway logs             |
...
4. Check logs: `matryoshka logs --follow`
...
5. Ensure the gateway is running: `matryoshka gateway status`
6. Check logs: `matryoshka logs --follow`
