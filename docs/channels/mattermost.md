---
summary: "Mattermost bot setup and MatryoshkaClaw config"
read_when:
...
```bash
matryoshka plugins install @matryoshka/mattermost
```

Local checkout (when running from a git repo):

```bash
matryoshka plugins install ./extensions/mattermost
```
...
4. Configure MatryoshkaClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, MatryoshkaClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, MatryoshkaClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as MatryoshkaClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to MatryoshkaClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from MatryoshkaClaw, not `404`.
...
- Approve via:
  - `matryoshka pairing list mattermost`
  - `matryoshka pairing approve mattermost <CODE>`
...
Use these target formats with `matryoshka message send` or cron/webhooks:
...
MatryoshkaClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), MatryoshkaClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, MatryoshkaClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and MatryoshkaClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="matryoshka-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"matryoshka-mattermost-interactions",
...
`matryoshka message send` and cron/webhook deliveries.
