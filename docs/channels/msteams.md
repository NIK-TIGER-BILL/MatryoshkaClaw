...
```bash
matryoshka plugins install @matryoshka/msteams
```

Local checkout (when running from a git repo):

```bash
matryoshka plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
MatryoshkaClaw will offer the local install path automatically.
...
3. Configure MatryoshkaClaw with those credentials.
...
- Talk to MatryoshkaClaw via Teams DMs, group chats, or channels.
...
- On startup, MatryoshkaClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring MatryoshkaClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `matryoshka-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `matryoshka plugins install @matryoshka/msteams`
   - From a local checkout: `matryoshka plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "MatryoshkaClaw in Teams", "full": "MatryoshkaClaw in Teams" },
...
MatryoshkaClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, MatryoshkaClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
MatryoshkaClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `matryoshka message poll --channel msteams --target conversation:<id> ...`
...
```bash
matryoshka message send --channel msteams \
...
matryoshka message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
matryoshka message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
matryoshka message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
matryoshka message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
