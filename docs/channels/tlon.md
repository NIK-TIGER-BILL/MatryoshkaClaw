...
Tlon is a decentralized messenger built on Urbit. MatryoshkaClaw connects to your Urbit ship and can
...
```bash
matryoshka plugins install @matryoshka/tlon
```

Local checkout (when running from a git repo):

```bash
matryoshka plugins install ./extensions/tlon
```
...
By default, MatryoshkaClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `matryoshka message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MatryoshkaClaw replies in-thread.
