    ...
    list: [
      {
        ...
        groupChat: {
          historyLimit: 50,
          mentionPatterns: ["@?matryoshka", "\\+?15555550123"],
        },
      },
    ],
  },
...
- The regexes are case-insensitive; they cover a display-name ping like `@matryoshka` and the raw number with or without `+`/spaces.
...
1. Add your WhatsApp account (the one running MatryoshkaClaw) to the group.
2. Say `@matryoshka …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@matryoshka` ping in the group and confirm a reply that references the sender name.
