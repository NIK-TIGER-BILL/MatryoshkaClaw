...
```bash
matryoshka plugins install @matryoshka/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
matryoshka plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "MatryoshkaClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `matryoshka pairing list nextcloud-talk`
  - `matryoshka pairing approve nextcloud-talk <CODE>`
