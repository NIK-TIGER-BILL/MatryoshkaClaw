...
Matrix is an open, decentralized messaging protocol. MatryoshkaClaw connects as a Matrix **user**
...
```bash
matryoshka plugins install @matryoshka/matrix
```

Local checkout (when running from a git repo):

```bash
matryoshka plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `matryoshka plugins install @matryoshka/matrix`
   - From a local checkout: `matryoshka plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: MatryoshkaClaw calls the same
...
- On first connection, MatryoshkaClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  MatryoshkaClaw logs a warning.
...
- Approve via:
  - `matryoshka pairing list matrix`
  - `matryoshka pairing approve matrix <CODE>`
...
- On startup, MatryoshkaClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
