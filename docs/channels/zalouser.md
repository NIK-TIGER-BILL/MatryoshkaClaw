...
read_when:
  - Setting up Zalo Personal for MatryoshkaClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside MatryoshkaClaw.
...
- Install via CLI: `matryoshka plugins install @matryoshka/zalouser`
- Or from a source checkout: `matryoshka plugins install ./extensions/zalouser`
...
```bash
matryoshka directory self --channel zalouser
matryoshka directory peers list --channel zalouser --query "name"
matryoshka directory groups list --channel zalouser --query "work"
```
...
- `matryoshka pairing list zalouser`
- `matryoshka pairing approve zalouser <code>`
...
- On startup, MatryoshkaClaw resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, MatryoshkaClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in MatryoshkaClaw state. Example:
...
- MatryoshkaClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, MatryoshkaClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `matryoshka channels logout --channel zalouser && matryoshka channels login --channel zalouser`
...
- The channel now runs fully in MatryoshkaClaw without external CLI binaries.
