...
If you do nothing, MatryoshkaClaw runs a single agent:
...
```bash
matryoshka agents list --bindings
```
...
```bash
matryoshka agents add coding
matryoshka agents add social
```
...
- If you later add the same binding for the same agent with an explicit account id, MatryoshkaClaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, MatryoshkaClaw falls back
...
```bash
matryoshka channels login --channel whatsapp --account personal
matryoshka channels login --channel whatsapp --account biz
```
