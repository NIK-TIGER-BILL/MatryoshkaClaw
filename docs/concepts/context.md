...
read_when:
  - You want to understand what “context” means in MatryoshkaClaw
...
“Context” is **everything MatryoshkaClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (MatryoshkaClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How MatryoshkaClaw builds the system prompt

The system prompt is **MatryoshkaClaw-owned** and rebuilt each run. It includes:
...
By default, MatryoshkaClaw injects a fixed set of workspace files (if present):
...
By default, MatryoshkaClaw uses the built-in `legacy` context engine for assembly and
compaction. If you install a plugin that provides `kind: "context-engine"` and
select it with `plugins.slots.contextEngine`, MatryoshkaClaw delegates context
