...
- `/usage cost` in chats: local cost summary aggregated from MatryoshkaClaw session logs.
- CLI: `matryoshka status --usage` prints a full per-provider breakdown.
- CLI: `matryoshka channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
