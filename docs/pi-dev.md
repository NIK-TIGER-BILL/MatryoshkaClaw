...
This guide summarizes a sane workflow for working on the pi integration in MatryoshkaClaw.
...
- Trigger the agent directly:
  - `pnpm matryoshka agent --message "Hello" --thinking low`
...
State lives under the MatryoshkaClaw state directory. Default is `~/.openclaw`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.
