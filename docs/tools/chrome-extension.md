---
summary: "Chrome extension: let MatryoshkaClaw drive your existing Chrome tab"
read_when:
...
The MatryoshkaClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate matryoshka-managed Chrome profile.
...
MatryoshkaClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the MatryoshkaClaw release (npm package) as static files. There is no separate “build” step.

After upgrading MatryoshkaClaw:

- Re-run `matryoshka browser extension install` to refresh the installed files under your MatryoshkaClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

MatryoshkaClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want MatryoshkaClaw to control.
...
- `ON`: attached; MatryoshkaClaw can drive that tab.
...
Debugging: `matryoshka sandbox explain`
...
`matryoshka browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `matryoshka browser extension install` first to copy the extension to a stable location under your MatryoshkaClaw state directory.
...
- **This is not isolated** like the dedicated matryoshka-managed profile.
