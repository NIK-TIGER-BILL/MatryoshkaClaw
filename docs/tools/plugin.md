---
summary: "MatryoshkaClaw plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends MatryoshkaClaw with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core MatryoshkaClaw yet (or you want to keep optional features out of your main
install).
...
```bash
matryoshka plugins list
```

2. Install an official plugin (example: Voice Call):

```bash
matryoshka plugins install @matryoshka/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@matryoshka/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@matryoshka/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@matryoshka/zalouser`
- [Matrix](/channels/matrix) — `@matryoshka/matrix`
- [Nostr](/channels/nostr) — `@matryoshka/nostr`
- [Zalo](/channels/zalo) — `@matryoshka/zalo`
- [Microsoft Teams](/channels/msteams) — `@matryoshka/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

MatryoshkaClaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
const result = await api.runtime.tts.textToSpeechTelephony({
  text: "Hello from MatryoshkaClaw",
  cfg: api.config,
});
```
...
Use SDK subpaths instead of the monolithic `matryoshka/plugin-sdk` import when
authoring plugins:

- `matryoshka/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `matryoshka/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `matryoshka/plugin-sdk/telegram` for Telegram channel plugins.
- `matryoshka/plugin-sdk/discord` for Discord channel plugins.
- `matryoshka/plugin-sdk/slack` for Slack channel plugins.
- `matryoshka/plugin-sdk/signal` for Signal channel plugins.
- `matryoshka/plugin-sdk/imessage` for iMessage channel plugins.
- `matryoshka/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `matryoshka/plugin-sdk/line` for LINE channel plugins.
- `matryoshka/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `matryoshka/plugin-sdk/acpx`, `matryoshka/plugin-sdk/bluebubbles`,
  `matryoshka/plugin-sdk/copilot-proxy`, `matryoshka/plugin-sdk/device-pair`,
  `matryoshka/plugin-sdk/diagnostics-otel`, `matryoshka/plugin-sdk/diffs`,
  `matryoshka/plugin-sdk/feishu`,
  `matryoshka/plugin-sdk/google-gemini-cli-auth`, `matryoshka/plugin-sdk/googlechat`,
  `matryoshka/plugin-sdk/irc`, `matryoshka/plugin-sdk/llm-task`,
  `matryoshka/plugin-sdk/lobster`, `matryoshka/plugin-sdk/matrix`,
  `matryoshka/plugin-sdk/mattermost`, `matryoshka/plugin-sdk/memory-core`,
  `matryoshka/plugin-sdk/memory-lancedb`,
  `matryoshka/plugin-sdk/minimax-portal-auth`,
  `matryoshka/plugin-sdk/nextcloud-talk`, `matryoshka/plugin-sdk/nostr`,
  `matryoshka/plugin-sdk/open-prose`, `matryoshka/plugin-sdk/phone-control`,
  `matryoshka/plugin-sdk/qwen-portal-auth`, `matryoshka/plugin-sdk/synology-chat`,
  `matryoshka/plugin-sdk/talk-voice`, `matryoshka/plugin-sdk/test-utils`,
  `matryoshka/plugin-sdk/thread-ownership`, `matryoshka/plugin-sdk/tlon`,
  `matryoshka/plugin-sdk/twitch`, `matryoshka/plugin-sdk/voice-call`,
  `matryoshka/plugin-sdk/zalo`, and `matryoshka/plugin-sdk/zalouser`.

Compatibility note:

- `matryoshka/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `matryoshka status`, `matryoshka status --all`,
  `matryoshka channels status`, `matryoshka channels resolve`, and doctor/config
...
4. Bundled extensions (shipped with MatryoshkaClaw, mostly disabled by default)

- `<matryoshka>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `matryoshka plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, MatryoshkaClaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. MatryoshkaClaw blocks candidates when:
...
Each plugin must include a `matryoshka.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `matryoshka.extensions`:
...
Security guardrail: every `matryoshka.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `matryoshka plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `matryoshka.channel` and
install hints via `matryoshka.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@matryoshka/nextcloud-talk",
  "matryoshka": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@matryoshka/nextcloud-talk",
...
MatryoshkaClaw can also merge **external channel catalogs** (for example, an MPM
...
contain `{ "entries": [ { "name": "@scope/pkg", "matryoshka": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, MatryoshkaClaw uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `matryoshka.plugin.json` (`configSchema`).
...
MatryoshkaClaw augments `uiHints` at runtime based on discovered plugins:
...
```bash
matryoshka plugins list
matryoshka plugins info <id>
matryoshka plugins install <path>                 # copy a local file/dir into ~/.openclaw/extensions/<id>
matryoshka plugins install ./extensions/voice-call # relative path ok
matryoshka plugins install ./plugin.tgz           # install from a local tarball
matryoshka plugins install ./plugin.zip           # install from a local zip
matryoshka plugins install -l ./extensions/voice-call # link (no copy) for dev
matryoshka plugins install @matryoshka/voice-call # install from npm
matryoshka plugins install @matryoshka/voice-call --pin # store exact resolved name@version
matryoshka plugins update <id>
matryoshka plugins update --all
matryoshka plugins enable <id>
matryoshka plugins disable <id>
matryoshka plugins doctor
```

`plugins update` only works for npm installs tracked under `plugins.installs`.
If stored integrity metadata changes between updates, MatryoshkaClaw warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `matryoshka voicecall`).
...
- Plugin-managed hooks show up in `matryoshka hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `matryoshka hooks`; enable/disable the plugin instead.
...
- When disabled, MatryoshkaClaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside MatryoshkaClaw (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `matryoshka models auth login --provider <id> [--method <id>]`
...
- `config`: The current MatryoshkaClaw config
...
- Main package: `matryoshka` (this repo)
- Plugins: separate npm packages under `@matryoshka/*` (example: `@matryoshka/voice-call`)

Publishing contract:

- Plugin `package.json` must include `matryoshka.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `matryoshka plugins install <npm-spec>` uses `npm pack`, extracts into `~/.openclaw/extensions/<id>/`, and enables it in config.
...
- CLI: `matryoshka voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `matryoshka.extensions` points at the built entrypoint (`dist/index.js`).
