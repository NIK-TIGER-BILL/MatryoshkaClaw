...
read_when:
  - Running MatryoshkaClaw Gateway in WSL2 while Chrome lives on Windows
...
- MatryoshkaClaw Gateway runs inside WSL2
...
Use the built-in `chrome` profile plus the MatryoshkaClaw Chrome extension.
...
- MatryoshkaClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, MatryoshkaClaw is not the problem yet.
...
Fix that before touching MatryoshkaClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point MatryoshkaClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting MatryoshkaClaw to succeed
...
```bash
matryoshka browser open https://example.com --browser-profile remote
matryoshka browser tabs --browser-profile remote
```

For the extension relay:

```bash
matryoshka browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `matryoshka browser tabs` returns the target
...
3. MatryoshkaClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug MatryoshkaClaw config or Control UI auth
