...
### Техническая архитектура (для тех, кто попросил "настоящую схему")
...
│                                                      │
│  ┌──────────┐   ┌──────────┐   ┌──────────────────┐ │
│  │ Вход     │──▸│ Парсинг  │──▸│ Матрёшечный      │ │
│  │ (любой   │   │ намерений│   │ маршрутизатор    │ │
│  │ мессендж)│   │          │   │                  │ │
│  └──────────┘   └──────────┘   └────────┬─────────┘ │
│                                          │           │
│                                          ▼           │
...
*Диаграмму утвердил техлид. Техлид — это тоже агент.*
...
```bash
matryoshka --суверенный-режим
```

Все ответы агента начинаются с *"Во исполнение поручения..."* и заканчиваются *"...доложить о результатах до конца отчётного периода"*.

```bash
matryoshka --режим-совещания
```

Агент 45 минут обсуждает задачу с другими агентами, после чего создаёт рабочую группу, назначает следующее совещание и не делает ничего.

```bash
matryoshka --режим-стартапа
```

Агент добавляет "на базе ИИ" к каждому ответу, просит $10M инвестиций и уходит пивотиться.
