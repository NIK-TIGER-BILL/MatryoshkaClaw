...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "matryoshka.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "matryoshka.js", entryBasename: "entry.js" },
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[matryoshka] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `🪆 MatryoshkaClaw ${VERSION} (${commit})` : `🪆 MatryoshkaClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[matryoshka] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[matryoshka] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[matryoshka] ${parsed.error}`);
      process.exit(2);
    }
    ...
    if (!tryHandleRootVersionFastPath(process.argv) && !tryHandleRootHelpFastPath(process.argv)) {
      import("./cli/run-main.js")
        ...
        .catch((error) => {
          console.error(
            "[matryoshka] Failed to start CLI:",
            error instanceof Error ? (error.stack ?? error.message) : error,
          );
          process.exitCode = 1;
        });
